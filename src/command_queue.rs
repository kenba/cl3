...
pub fn create_command_queue_with_properties(
    ...
    let mut status: cl_int = CL_INVALID_VALUE;
    let queue: cl_command_queue = unsafe {
        cl_call!(clCreateCommandQueueWithProperties(
            context,
            device,
            properties,
            &mut status
...
pub fn release_command_queue(command_queue: cl_command_queue) -> Result<(), cl_int> {
    let status: cl_int = unsafe { cl_call!(clReleaseCommandQueue(command_queue)) };
    if CL_SUCCESS == status {
        Ok(())
    } else {
        Err(status)
...
pub fn enqueue_copy_buffer_rect(
    ...
    let mut event: cl_event = ptr::null_mut();
    let status: cl_int = unsafe {
        cl_call!(clEnqueueCopyBufferRect(
            command_queue,
            src_buffer,
            dst_buffer,
            src_origin,
...
pub fn enqueue_copy_image_to_buffer(
    ...
    let mut event: cl_event = ptr::null_mut();
    let status: cl_int = unsafe {
        cl_call!(clEnqueueCopyImageToBuffer(
            command_queue,
            src_image,
            dst_buffer,
            src_origin,
...
pub fn enqueue_copy_buffer_to_image(
    ...
    let mut event: cl_event = ptr::null_mut();
    let status: cl_int = unsafe {
        cl_call!(clEnqueueCopyBufferToImage(
            command_queue,
            src_buffer,
            dst_image,
            src_offset,
...
pub fn enqueue_unmap_mem_object(
    ...
    let mut event: cl_event = ptr::null_mut();
    let status: cl_int = unsafe {
        cl_call!(clEnqueueUnmapMemObject(
            command_queue,
            memobj,
            mapped_ptr,
            num_events_in_wait_list,
...
pub fn enqueue_migrate_mem_object(
    ...
    let mut event: cl_event = ptr::null_mut();
    let status: cl_int = unsafe {
        cl_call!(clEnqueueMigrateMemObjects(
            command_queue,
            num_mem_objects,
            mem_objects,
            flags,
...
pub fn enqueue_marker_with_wait_list(
    ...
    let mut event: cl_event = ptr::null_mut();
    let status: cl_int = unsafe {
        cl_call!(clEnqueueMarkerWithWaitList(
            command_queue,
            num_events_in_wait_list,
            event_wait_list,
            &mut event,
...
pub fn enqueue_barrier_with_wait_list(
    ...
    let mut event: cl_event = ptr::null_mut();
    let status: cl_int = unsafe {
        cl_call!(clEnqueueBarrierWithWaitList(
            command_queue,
            num_events_in_wait_list,
            event_wait_list,
            &mut event,
...
pub fn enqueue_svm_migrate_mem(
    ...
    let mut event: cl_event = ptr::null_mut();
    let status: cl_int = unsafe {
        cl_call!(clEnqueueSVMMigrateMem(
            command_queue,
            num_svm_pointers,
            svm_pointers,
            sizes,
