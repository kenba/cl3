...
pub fn get_device_ids_from_dx9_intel(
    ...
    let status: cl_int = unsafe {
        cl_call!(cl_dx9_media_sharing::clGetDeviceIDsFromDX9INTEL(
            platform,
            dx9_device_source,
            dx9_object,
            dx9_device_set,
...
pub fn create_from_dx9_media_surface_intel(
    ...
    let mem = unsafe {
        cl_call!(cl_dx9_media_sharing::clCreateFromDX9MediaSurfaceINTEL(
            context,
            flags,
            resource,
            shared_handle,
...
pub fn enqueue_acquire_dx9_objects_intel(
    ...
    let status: cl_int = unsafe {
        cl_call!(cl_dx9_media_sharing::clEnqueueAcquireDX9ObjectsINTEL(
            command_queue,
            num_objects,
            mem_objects,
...
pub fn enqueue_release_dx9_objects_intel(
    ...
    let mut event: cl_event = ptr::null_mut();
    let status: cl_int = unsafe {
        cl_call!(cl_dx9_media_sharing::clEnqueueReleaseDX9ObjectsINTEL(
            command_queue,
            num_objects,
            mem_objects,
            num_events_in_wait_list,
