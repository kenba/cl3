...
pub fn update_mutable_commands_khr(
    ...
    let status: cl_int = unsafe { clUpdateMutableCommandsKHR(command_buffer, mutable_config) };
    if CL_SUCCESS == status {
        Ok(())
    } else {
        Err(status)
...
pub fn reimport_semaphore_sync_fd(
    ...
    let status: cl_int = unsafe { clReImportSemaphoreSyncFdKHR(sema_object, reimport_props, fd) };
    if CL_SUCCESS == status {
        Ok(())
    } else {
        Err(status)
